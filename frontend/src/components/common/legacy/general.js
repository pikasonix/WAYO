...
// Example helper: fetch instances via backend
async function fetchInstancesExample() {
    const { data, error } = await db.select('instances');
    if (error) {
...
//dropdowns
// click_open_close_dropdown(); // Called prematurely, dropdown-btn elements are created dynamically later
